...
import { WalletConnectionError, HMY_TESTNET_URL } from "../../constants";
...
const { store } = Store;
const tParams = { receiver: "", amount: 0 };
const swapParams = { swapAmount: 0 };
...
export default function TokenSwap(props) {
  ...
  const renderSnackbar = () => (
    <Snackbar type={snackbarType} message={snackbarMessage} open />
  );
  ...
  async function getData(addr) {
    ...
    try {
      contract.methods.balanceOf(addr).call((err, result) => {
        error = err || setNewData(result / 1e18);
      });
    } catch {
    ...
    if (!loading) {
      ...
      try {
        ...
        if (error instanceof WalletConnectionError) {
          ...
          setSnackbarMessage(
            `${url}2. An error occurred :(. Please try again!${error}`
          );
        }
